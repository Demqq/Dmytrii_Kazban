...
let btnValidation = document.getElementById('validate');
let inputs = document.querySelectorAll('input');

btnValidation.addEventListener('click', function (event) {
    event.preventDefault();
    if (myForm.checkValidity() == true) {
        alert("Our form has no errors")
    } else {
        alert("Our form has errors, can you solve it?")
    }
});
    ...
    inputs[i].addEventListener("input", function () {
        if (inputs[i].id == "login") {
            if (inputs[i].value.length < 2) {
                inputs[i].setCustomValidity("Как тебя зовут, дружище?!");
            ...
            if (inputs[i].value.indexOf("@") == -1 ) {
                inputs[i].setCustomValidity("Ну и зря, не получишь бандероль с яблоками!");
                inputs[i].reportValidity();
            } else {
                inputs[i].setCustomValidity('');
                inputs[i].reportValidity();
            ...
            if (inputs[i].value == "" ) {
                inputs[i].setCustomValidity("Я никому не скажу наш секрет");
                inputs[i].reportValidity();
            } else {
                inputs[i].setCustomValidity('');
                inputs[i].reportValidity();
        ...
        } else if (inputs[i].id == "apples") {
            if (inputs[i].value == "0" ) {
                inputs[i].setCustomValidity("Ну хоть покушай немного... Яблочки вкусные");
                inputs[i].reportValidity();
            } else {
                inputs[i].setCustomValidity('');
                inputs[i].reportValidity();
        ...
        } else if (inputs[i].id == "thanks") {
            if (inputs[i].value !== "спасибо" ) {
                inputs[i].setCustomValidity("Фу, неблагодарный(-ая)!");
                inputs[i].reportValidity();
            } else {
                inputs[i].setCustomValidity('');
                inputs[i].reportValidity();
        ...
        } else if (inputs[i].id == "check") {
            if (inputs[i].checked !== true ) {
                inputs[i].setCustomValidity("Необразованные живут дольше! Хорошо подумай!");
                inputs[i].reportValidity();
            } else {
                inputs[i].setCustomValidity('');
                inputs[i].reportValidity();
